...
import { requestCentreVaccineStock, updateCentreVaccineStock, listAssignedCentreVaccines } from "../../../services/centreVaccineService";
...
// Subcomponents to isolate modal/toast JSX
const RequestModal = ({ requestModal, closeRequest, confirmRequest, setRequestModal }) => (
  <AnimatePresence>
    {requestModal.open && (
      <motion.div
        key="request-overlay"
      ...
      >
        ...
        >
          <div className="flex items-center gap-3 mb-2">
            ...
            <h4 className="text-lg font-semibold text-[#081F2E]">Confirm Stock Request</h4>
          </div>
          <p className="text-sm text-[#0c2b40]/80">
            Do you want to request stock for
            <span className="font-semibold text-[#081F2E]"> {requestModal.item?.vaccineName}</span> (
            <span className="font-mono">{requestModal.item?.vaccineId}</span>)?
          </p>
          <div className="mt-3">
            <input
              type="number"
              ...
              onChange={(e) => setRequestModal((m) => ({ ...m, amount: e.target.value }))}
...
const AddStockModal = ({ addModal, closeAdd, confirmAdd, setAddModal }) => (
  <AnimatePresence>
    {addModal.open && (
      <motion.div
        key="add-overlay"
      ...
      >
        ...
        >
          <div className="flex items-center gap-3 mb-2">
            ...
            <h4 className="text-lg font-semibold text-[#081F2E]">Add New Stock</h4>
          </div>
          <p className="text-sm text-[#0c2b40]/80">
            Enter number of new ampules for
            <span className="font-semibold text-[#081F2E]"> {addModal.item?.vaccineName}</span> (
          ...
          <div className="mt-3">
            <input
              ...
              onChange={(e) => setAddModal((m) => ({ ...m, amount: e.target.value }))}
              className="w-full rounded-xl border border-[#081F2E]/20 px-3 py-2 text-sm text-[#081F2E] focus:outline-none focus:ring-2 focus:ring-[#081F2E]/40"
              placeholder="e.g., 50"
            />
          </div>
...
const CentreStock = () => {
  ...
  // Fetch assigned vaccines once when the component mounts
  useEffect(() => {
  ...
  const confirmRequest = async () => {
    ...
    if (!item) return;
    try {
      await requestCentreVaccineStock(item.id || item.centreVaccineId || item.vaccineId, amt);
      setStockRows((prev) =>
        prev.map((r) =>
      ...
      setFeedback({ type: "error", message: "Failed to request stock" });
    } finally {
  ...
  const confirmAdd = async () => {
    ...
    try {
      await updateCentreVaccineStock(item.id || item.centreVaccineId || item.vaccineId, amt);
      setStockRows((prev) =>
        prev.map((r) =>
          r.vaccineId === item.vaccineId
    ...
    >
      ...
      >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
          {loading ? (
            <div className="text-center col-span-full py-10 text-[#0c2b40]/70">Loading...</div>
          ) : (
