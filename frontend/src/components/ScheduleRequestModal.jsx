...
const ScheduleRequestModal = ({ isOpen, onClose, onConfirm, centre, vaccine }) => {
  ...
  const minDate = today;
  const maxDate = startOfDay(new Date(today.getFullYear(), today.getMonth() + 1, today.getDate()));
  const minMonthStart = startOfDay(new Date(minDate.getFullYear(), minDate.getMonth(), 1));
  const maxMonthStart = startOfDay(new Date(maxDate.getFullYear(), maxDate.getMonth(), 1));

  const initialMonthStart = startOfDay(new Date(today.getFullYear(), today.getMonth(), 1));
                ...
                <div className="grid grid-cols-1 gap-3">
                  <div className="rounded-xl bg-[#081F2E]/5 p-3 ring-1 ring-[#081F2E]/10">
                      ...
                      <div className="flex items-center gap-1">
                        <button
                          ...
                          className={`inline-flex h-8 w-8 items-center justify-center rounded-lg ${
                            canGoPrev ? "bg-[#081F2E]/10 text-[#081F2E] hover:bg-[#081F2E]/15" : "bg-[#081F2E]/5 text-[#0c2b40]/40 cursor-not-allowed"
                          }`}
                        ...
                        <button
                          ...
                          onClick={goNext}
                          className={`inline-flex h-8 w-8 items-center justify-center rounded-lg ${
                            canGoNext ? "bg-[#081F2E]/10 text-[#081F2E] hover:bg-[#081F2E]/15" : "bg-[#081F2E]/5 text-[#0c2b40]/40 cursor-not-allowed"
                          }`}
                    ...
                    <div className="mt-2 flex items-center justify-between">
                      <div className="text-xs font-medium text-[#0c2b40]/70">{monthLabel}</div>
                      <div className="text-xs text-[#0c2b40]/60">Selected: {selectedDateStr}</div>
                    </div>

                    <div className="mt-3">
                      <div className="grid grid-cols-7 gap-1 mb-2">
                        {dayNames.map((d) => (
                          <div key={d} className="text-center text-xs font-semibold text-[#0c2b40]/70">
                            {d}
                          </div>
                        ))}
                      </div>
